...
def handler(event: S3ObjectLambdaEvent, context: LambdaContext):
    ...
    print(
        f"Environment - BUCKET_NAME (s3_resource_class): {s3_resource_class.bucket_name}")
    ...
    try:
        generate_doc_links(doc_links_json, uploaded_object_key,
                           project, s3_resource_class)
        path_fetched_object_from_s3 = fetch_uploaded_object(
            uploaded_object_key, s3_resource_class)
        with tempfile.TemporaryDirectory() as tmp_dir:
            path_dir_to_extract_archive_into = Path(tmp_dir)
            s3_key_project = f"{projects_space}/{project.full_name}"
            unzip_validate_upload(s3_resource_class, s3_key_project,
                                  path_fetched_object_from_s3, path_dir_to_extract_archive_into)
...
def fetch_uploaded_object(uploaded_object_key: str, s3_resource: LambdaS3Class) -> Path:
    ...
    try:
        ...
        s3_resource.bucket.download_file(
            uploaded_object_key, str(local_zip_path))
        print(f"Downloaded '{uploaded_object_key}' from S3 to a local temp '{
              local_zip_path}'")
...
def unzip_validate_upload(s3_resource: LambdaS3Class, s3_key_project: str, zip_from_s3_path: Path, path_dir_to_extract_archive_into: Path):
    ...
    try:
        shutil.unpack_archive(
            zip_from_s3_path, path_dir_to_extract_archive_into)
        ...
        if validator.validate():
            ...
            for root, folders, files in path_dir_to_extract_archive_into.walk():
                for name in files:
                    ...
                    file_s3_key = f"{
                        s3_key_project}/{str(file_path.relative_to(path_dir_to_extract_archive_into))}"
                ...
                for folder in folders:
                    print(f"There is a folder {
                          folder} existing after zip extraction")
        else:
            ...
            print(f"Validation failed. Uploading error index.html with report to {
                  error_index_html_s3_key}")
