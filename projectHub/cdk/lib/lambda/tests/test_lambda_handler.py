...
RESOURCES_DOCS_ZIP_SAMPLE_NOT_VALID: Path = Path(CURRENT_PATH / "resources" / "noDocsFolder-1.0.0.zip")
...
@pytest.fixture
def copy_valid_resources_docs(tmp_path):
    """
...
@pytest.fixture
def copy_not_valid_resources_docs(tmp_path):
    """
    Copies NOT VALID resource zip files to temporary directories within tmp_path.
    ...
    """
    resources_docs = {
        "nv": RESOURCES_DOCS_ZIP_SAMPLE_NOT_VALID,
    }
...
class TestUnzipValidateUpload:
    def test_check_prerequisites(self, copy_valid_resources_docs):
        """
        ...
        for resource_dir, copied_file_path in copy_valid_resources_docs.items():
    ...
    def test_unpacked_doc_zip_has_desired_amount_of_files(self, s3, copy_valid_resources_docs, tmp_path):
        """
        Test unpacking document zips and comparing the file count with the extracted contents.
        """
        for project_key, zip_path in copy_valid_resources_docs.items():
    ...
    def test_s3_upload_file_count(self, s3, copy_valid_resources_docs, tmp_path):
        """
    ...
    def test_validation_passed_for_correct_zip(self, s3, copy_not_valid_resources_docs, tmp_path):
        """
        Test if validation functionality worked correctly for not valid documentations uploaded.
        """
        for project_key, zip_path in copy_not_valid_resources_docs.items():
            base_extract_path = tmp_path / "extracted_docs" / project_key
            base_extract_path.mkdir(parents=True, exist_ok=True)
